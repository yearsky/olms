...
class Crud_model extends CI_Model {
    ...
    public function get_class($param1 = "") {
        if ($param1 != "") {
            $this->db->where('id_kelas', $param1);
        }
        $this->db->where('parent', 0);
    ...
    public function edit_class($param1) {
        $data['name']   = html_escape($this->input->post('name'));
        $data['parent'] = html_escape($this->input->post('parent'));
    ...
    public function delete_class($class_id) {
        $this->db->where('id_kelas', $class_id);
    ...
    public function update_course($course_id, $type = "") {
        ...
        $data['description'] = $this->input->post('description');
        $data['outcomes'] = $outcomes;
        // $data['kelas_id'] = $this->input->post('class_id');
        $data['sub_kelas_id'] = $this->input->post('sub_class_id');
        $data['teachers_id'] = $this->input->post('teachers');
    ...
    public function filter_course_for_backend($category_id, $instructor_id, $class_id, $status) {
        if ($category_id != "all") {
        ...
        if ($class_id != "all") {
            $this->db->where('sub_kelas_id', $class_id);
        }


        if ($instructor_id != "all") {
            $this->db->where('teachers_id', $instructor_id);
        }
